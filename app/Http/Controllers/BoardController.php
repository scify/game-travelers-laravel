...
class BoardController extends Controller
{
    ...
    public function play(Request $request, int $player_id, int $game_id)
    {
        ...
        // playerAudio add-on for passing values to audio.js via window.Laravel:
        $playerAudio = [
            'playerMusicVolume' => $player_data['music_volume'],
            'playerSoundVolume' => $player_data['sound_volume'],
    ...
    public function fromVue(Request $request)
    {
        ...
        if (sizeof($games) != 1) {
            ...
            } elseif ($game_phase == 1) { // asked to roll dice and compute new target position
            ...
            } elseif ($game_phase == 2) { // move performed by die roll in the front-end
            ...
            } elseif ($game_phase == 3) { //move performed by card
                if ($game_phase != $db_game_phase) {
                    $first_player_turn_new_value = true;
                    if ($game_mode > 1) { // switch player only if the game is not solo
                        if ($first_player_turn) {
