...
window.addEventListener("load", function () {
	...
	const groupSetters = document.querySelectorAll(
		"input[data-role='groupSetter']"
	);
			...
			if (groupSetter.checked) {
				...
				if (enableBtns) {
					for (const enableBtn of enableBtns) {
						...
						dataKeySelected =
							enableBtn.getAttribute("data-key-selected");
						if (dataKeySelected) {
							enableBtn.textContent =
								enableBtn.getAttribute("data-key-selected");
						} else {
							enableBtn.textContent =
								enableBtn.getAttribute("data-key-default");
						}
					}
				}
